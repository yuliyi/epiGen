...
sc.tl.rank_genes_groups(atac_truth_filter, groupby='celltype', method='wilcoxon', use_raw=False, pts=True)
...
for ct in celltype_order:
    ...
    truth_top_peaks[ct] = temp_df['peak'].values
    truth_top_lfcs[ct] = temp_df['logfc'].values
...
# Evaluation Function
# ---------------------------------------------------------
def evaluate_prediction(pred_adata, truth_adata, truth_top_peaks, truth_top_lfcs, method_name):
    """
    ...
    sc.tl.rank_genes_groups(pred_adata, groupby='celltype', method='wilcoxon', use_raw=False)
    ...
    for mct in truth_top_peaks.keys():
        t_peaks = truth_top_peaks[mct]
        t_lfc = truth_top_lfcs[mct]
...
evaluate_prediction(atac_scratch, atac_truth, truth_top_peaks, truth_top_lfcs, "scratch")

# nofinetune
atac_noff = ad.read_h5ad(prefix + f"/prediction/allchr_GSE234521_bins_nofinetune.h5ad")
evaluate_prediction(atac_noff, atac_truth, truth_top_peaks, truth_top_lfcs, "nofinetune")

# finetune 0.1
atac_ff = ad.read_h5ad(prefix + f"/prediction/allchr_GSE234521_bins_finetune_0.1_0.8.h5ad")
evaluate_prediction(atac_ff, atac_truth, truth_top_peaks, truth_top_lfcs, "finetune 0.1_0.8")

# MIDAS 
midas_infer = ad.read_h5ad(prefix + f"/midas/guangshi_atlas_GSE234521/atlas_b_pred_GSE234521.h5ad")
evaluate_prediction(midas_infer, atac_truth, truth_top_peaks, truth_top_lfcs, "MIDAS")