...
def parse_cal_label_args():
    ...
    parser.add_argument('--tis', type=str, default='breast', help='Specific tissue to analyze.')
    parser.add_argument('--ct', type=str, default='Tumor', help='Target cell type (e.g., Tumor or Normal).')
...
def parse_args():
    ...
    parser.add_argument('--data_dir', default='/path', help='Input data path.')
    ...
    parser.add_argument('--fold', type=str, default='tme', help='Name of the test dataset or cross-validation dataset.')
    ...
    parser.add_argument('--tis', type=str, default='breast', help='Specific tissue.')
    parser.add_argument('--ct', type=str, default='Tumor', help='Target cell type (e.g., Tumor or Normal).')
...
def finetune_batch_embedding(model, num_new_batches, dataloader, rna_batch, batch_int, device, lr, epochs=50):
    ...
    model.batch_embed = expand_batch_embedding(model.batch_embed, num_old_batches=222, num_new_batches=num_new_batches)
...
def simulate_perturbation(model, test_loader, rna_test_mat, rna_test_original, batch_test, 
    ...
    if permutation:
        ...
        group_col = rna_test_mat.obs['cell_type'].values
