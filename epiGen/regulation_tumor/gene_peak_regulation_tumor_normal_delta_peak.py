...
def calculate_delta_aupr_for_celltype(delta_adata, delta_adata_permutation, gene_name, gene_chrom, gene_start, gene_end,
    ...
    for ct in celltypes:
        ...
        if use_absolute_delta:
            ...
            celltype_delta = adata_ct.X.toarray().mean(axis=0)
            celltype_delta_perm = adata_perm_ct.X.toarray().mean(axis=0)
...
for gene_name in gene_pd[ct].dropna().values[:top]:
    if not os.path.exists(f"{data_dir}/perturbation_tumor/gene_peak_links/{tis}_{ct}_{gene_name}_nearby_links.csv"):
        ...
        links = pd.read_csv(data_dir + f"/perturbation_tumor/gene_peak_links/{tis}_{ct}_{gene_name}_nearby_links.csv")
