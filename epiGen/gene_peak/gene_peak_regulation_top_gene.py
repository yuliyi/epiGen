...
def calculate_celltype_specific_correlation_with_labels(rna_adata, atac_adata, gene_name='LEF1', 
                                                       ...
                                                       use_absolute=True, min_pct=0.05,
...
def calculate_delta_aupr_for_celltype(delta_adata, delta_adata_permutation, pred_correlations, labels, gene_name, gene_chrom, gene_start, gene_end,
                                      celltype_key='cell_type', ct='T', distances=[5e5, 3e5, 1e5],
    ...
    # Establish baseline expected perturbation from permutations
    ...
    print(f'avg_perturbation: {avg_perturbation}')
    ...
    if ct in labels.columns:
        ...
        for idx, dis in enumerate(distances):
            ...
            delta_values_binary_range = delta_values_binary[bool_peak[idx]]
            aligned_data = pd.DataFrame({
                'label': ct_labels[bool_peak[idx]].values,
                'delta': delta_values_range,
                'delta_binary': delta_values_binary_range,
                'preds': ct_preds[bool_peak[idx]].values
            }).dropna()
            
            if len(aligned_data) < 10:
                print(f"Warning: Insufficient data for cell type {ct}, skipping")
                return None
...
rna_list = ad.read_h5ad(data_dir + "/rna_data/rna_independence_GSE234521.h5ad")
atac_list = ad.read_h5ad(data_dir + f"/atac_data/atac_independence_GSE234521.h5ad")
celltype = pd.read_csv(os.path.join(data_dir + "/statistic/celltype_GSE234521.csv"), index_col=0)
...
for gene_name in gene_pd[ct].dropna().values[:top]:
    ...
    gene_atac_delta_adata = ad.read_h5ad(data_dir + f"/perturbation/gene_peak_regulation/atac_delta_GSE234521_{ct}_{gene_name}_{gene_chrom}.h5ad")
    gene_atac_delta_adata_permutation = ad.read_h5ad(data_dir + f"/perturbation/gene_peak_regulation_permutation/atac_delta_GSE234521_{ct}_{gene_name}_{gene_chrom}_permutation.h5ad")
    ...
    pred_list = np.load(f"{data_dir}/prediction/{gene_chrom}_allGSE234521_preds_nofinetune.npy")
    ...
    pred_correlations, _, _, _ = \
        calculate_celltype_specific_correlation_with_labels(
            ...
            use_absolute, min_pct=0, pval_threshold=pval_threshold,
            corr_threshold=corr_threshold
        )
    ...
    aupr_delta_labels = calculate_delta_aupr_for_celltype(
        gene_atac_delta_adata, gene_atac_delta_adata_permutation, pred_correlations, labels, gene_name, gene_chrom, gene_start, gene_end, celltype_key, ct, distances
    )
