...
def main(args, ct, chr):
    ...
    atac_delta_adata.write_h5ad(data_dir + f"/perturbation/gene_peak_regulation_permutation/atac_delta_{ct}_{gene_name}_{chr}_permutation.h5ad", compression='gzip')
    del model
    torch.cuda.empty_cache()
...
if __name__ == '__main__':
    ...
    for gene_name in gene_pd[ct].dropna().values[:20]:
        ...
        if not os.path.exists(f"{args.data_dir}/perturbation/gene_peak_regulation_permutation/atac_delta_{ct}_{gene_name}_{gene_chrom}_permutation.h5ad"):
