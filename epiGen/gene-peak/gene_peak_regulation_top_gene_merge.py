...
def merge_fragment(adata, bin_size=10000):
    ...
    print(f"pre-merge raw adata: {adata.shape}")
    print(f"post-merge raw adata: {adata_bin.shape}")
    return adata_bin, mapping_matrix, new_var


def merge_fragment_pred(adata, mapping_matrix, new_var):
    """
    Merge pred ATAC peaks into larger fixed-size genomic bins (e.g., 10kb).
    """
    # Matrix multiplication: (n_cells, n_old_peaks) @ (n_old_peaks, n_new_bins) = (n_cells, n_new_bins)
    new_X = adata.X @ mapping_matrix
    new_X.data = np.ones_like(new_X.data, dtype=np.int8)  # Binarize accessible bins
    ...
    print(f"pre-merge pred adata: {adata.shape}")
    print(f"post-merge pred adata: {adata_bin.shape}")
    return adata_bin
...
celltype = pd.read_csv(data_dir + "/statistic/celltype.csv", index_col=0)
...
rna_test_mat = rna_list[rna_list.obs.ct_a.isin(celltypes)].copy()
atac_list = atac_list[atac_list.obs.ct_a.isin(celltypes)].copy()
sc.pp.normalize_total(rna_test_mat, 1e4)
sc.pp.log1p(rna_test_mat)
atac_list_bin, mapping_matrix, new_var = merge_fragment(atac_list, bin_size)
atac_list_bin.X = tfidf_seurat(atac_list_bin)
pred_list = ad.read_h5ad(f"{data_dir}/prediction/allchr_bins.h5ad")
pred_list.obs = celltype
pred_list = pred_list[pred_list.obs.ct_a.isin(celltypes)].copy()
pred_list_bin = merge_fragment_pred(pred_list, mapping_matrix, new_var)
pred_list_bin.X = tfidf_seurat(pred_list_bin)
...
for gene_name in gene_pd[ct].dropna().values[:top]:
    ...
    atac_test_mat = atac_list_bin[:, peaks].copy()
    pred_test_mat = pred_list_bin[:, atac_test_mat.var_names].copy()
    gene_atac_delta_adata = gene_atac_delta_adata[gene_atac_delta_adata.obs.ct_a==ct]
    gene_atac_delta_adata_permutation = gene_atac_delta_adata_permutation[gene_atac_delta_adata_permutation.obs.ct_a==ct]
    ...
    labels.to_csv(data_dir + f"/perturbation/gene_peak_nearby_merge/{gene_name}_nearby_labels_{bin_size}.csv")
    ...
    pred_correlations.to_csv(data_dir + f"/perturbation/gene_peak_nearby_merge/{gene_name}_nearby_pred_correlations_{bin_size}.csv")
...
aupr_result.to_csv(data_dir + f"/perturbation/gene_peak_nearby_merge/nearby_aupr_{ct}_permutation_{bin_size}.csv")